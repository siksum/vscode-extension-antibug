...
class SecurityAnalysisViewProvider extends webview_1.default {
    constructor({ extensionUri, viewType, }) {
        super({
            extensionUri,
            viewType,
        });
    ...
    resolveWebviewView(webviewView, context, token) {
        ...
        const htmlPath = this.getPath(webviewView.webview, "template", "security-analysis", "index.ejs").fsPath;
        const style = this.getPath(webviewView.webview, "style", "security-analysis", "index.css");
        const controller = this.getPath(webviewView.webview, "controller", "security-analysis", "index.js");
        const options = [
            ...
            switch (type) {
                ...
                case "RunAnalysis": {
                    ...
                    if (files === "Select a file to analyze") {
                        vscode.window
                            .showInformationMessage("Please select a file to analyze.", "확인", "취소")
                            .then((value) => {
                            if (value === "확인") {
                        ...
                        if (result.match(/Detecting specific vulnerabilities/)) {
                            ...
                            if (OutputDirectoryMatch && OutputDirectoryMatch.length > 1) {
                                ...
                                this.auditReportEN = path.join(outputDirectoryPath, "/audit_report", filename + "_en.md");
                                this.auditReportKR = path.join(outputDirectoryPath, "/audit_report", filename + "_kr.md");
                                // Path 값 설정
                                const contractAnalysisResultPath = outputDirectoryPath +
                                    "/contract_analysis_json_results/" +
                                    filename +
                                    ".json";
                                const detectorResultPath = outputDirectoryPath +
                                    "/detector_json_results/" +
                                    filename +
                                    "_kr.json";
                                const callGraphResultPath = outputDirectoryPath +
                                    "/call_graph_json_results/" +
                                    filename +
                                    ".json";
                                const auditReportPath = outputDirectoryPath +
                                    "/call_graph_results/" +
                                    "call-graph.png";
                                const panelProvider = new analysis_result_1.default({
                                    extensionUri: this.extensionUri,
                                    ...
                                    switch (type) {
                                        ...
                                        case "ExtractAuditReport": {
                                            ...
                                            if (this.auditReportKR && this.auditReportEN) {
                                                await this.ExtractAuditReport(this.auditReportKR, isKoreanSelected);
                                                await this.ExtractAuditReport(this.auditReportEN, isEnglishSelected);
                                            }
                                            else {
                                                vscode.window
                                        ...
                                        case "codeLinechecked": {
                                            ...
                                            const lineNumber = parseInt(lineParts[lineParts.length - 1]);
                                            ...
                                            const decorationType = decorationTypes[impact] || decorationTypes.default;
                                            const openedDoc = vscode.workspace.textDocuments.find((doc) => doc.fileName === files);
                                            if (openedDoc) {
                                                vscode.window
                                                ...
                                                vscode.workspace
                                                    ...
                                                    console.error(error);
                                                    vscode.window.showErrorMessage("An error occurred while opening the file.");
                                                });
                                            }
                                            break;
                                        }
                                ...
                                vscode.window
                                    .showInformationMessage("Vulnerabilities have not been detected.", "확인", "취소")
                                    .then((value) => {
                                    if (value === "확인") {
                                        vscode.commands.executeCommand("workbench.action.problems.focus");
                                    }
    ...
    async analysis(rule, filePath) {
        return new Promise((resolve, reject) => {
            (0, child_process_1.exec)(`antibug detect ${rule} ${filePath}`, (error, stdout, stderr) => {
                if (error) {
    ...
    async ExtractAuditReport(filePath, view) {
        ...
        if (workspaceFolders && workspaceFolders.length > 0) {
            ...
            if (view) {
                vscode.commands.executeCommand("markdown.showPreviewToSide", vscode.Uri.file(newFilePath));
            }
        }
    }
}
exports.default = SecurityAnalysisViewProvider;
