...
import { Account, Address, BIGINT_0, BIGINT_8, GWEI_TO_WEI, KECCAK256_RLP, bigIntToBytes, bytesToHex, concatBytes, equalsBytes, hexToBytes, intToBytes, setLengthLeft, short, unprefixedHexToBytes, } from '@ethereumjs/util';
import debugDefault from 'debug';
import { Bloom } from './bloom/index.js';
const { debug: createDebugLogger } = debugDefault;
const debug = createDebugLogger('vm:block');
const parentBeaconBlockRootAddress = Address.fromString('0x000F3df6D732807Ef1319fB7B8bB8522d0Beac02');
 ...
 * @ignore
 */
export async function runBlock(opts) {
    ...
    if (this.DEBUG) {
        debug('-'.repeat(100));
        debug(`Running block hash=${bytesToHex(block.hash())} number=${block.header.number} hardfork=${this.common.hardfork()}`);
    }
...
export async function accumulateParentBeaconBlockRoot(root, timestamp) {
