...
function execHardfork(hardfork, preMergeHf) {
    return hardfork !== common_1.Hardfork.Paris ? hardfork : preMergeHf;
}
/**
 * @ignore
 */
async function runTx(opts) {
    ...
    opts.block = opts.block ?? block_1.Block.fromBlockData({}, { common: this.common });
...
async function _runTx(opts) {
    ...
    if (this.common.isActivatedEIP(2929) === true) {
        ...
        if (tx.to !== undefined) {
            ...
            this.evm.journal.addAlwaysWarmAddress((0, util_1.bytesToUnprefixedHex)(tx.to.bytes));
        }
        if (this.common.isActivatedEIP(3651) === true) {
            this.evm.journal.addAlwaysWarmAddress((0, util_1.bytesToUnprefixedHex)(block.header.coinbase.bytes));
        }
    }
    ...
    if (this.common.isActivatedEIP(3607) === true &&
        !(0, util_1.equalsBytes)(fromAccount.codeHash, util_1.KECCAK256_NULL)) {
    ...
    let gasRefund = results.execResult.gasRefund ?? util_1.BIGINT_0;
...
async function generateTxReceipt(tx, txResult, cumulativeGasUsed, blobGasUsed, blobGasPrice) {
    ...
    if (!tx.supports(tx_1.Capability.EIP2718TypedTransaction)) {
        // Legacy transaction
        if (this.common.gteHardfork(common_1.Hardfork.Byzantium) === true) {
            // Post-Byzantium
            receipt = {
