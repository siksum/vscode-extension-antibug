...
exports.encodeReceipt = exports.rewardAccount = exports.calculateMinerReward = exports.accumulateParentBeaconBlockRoot = exports.runBlock = void 0;
...
const parentBeaconBlockRootAddress = util_1.Address.fromString('0x000F3df6D732807Ef1319fB7B8bB8522d0Beac02');
...
async function runBlock(opts) {
    ...
    if (this.DEBUG) {
        debug('-'.repeat(100));
        debug(`Running block hash=${(0, util_1.bytesToHex)(block.hash())} number=${block.header.number} hardfork=${this.common.hardfork()}`);
    }
    ...
    if (this.common.hardforkIsActiveOnBlock(common_1.Hardfork.Dao, block.header.number) === true &&
        block.header.number === this.common.hardforkBlock(common_1.Hardfork.Dao)) {
        if (this.DEBUG) {
    ...
    if (generateFields) {
        ...
        block = block_1.Block.fromBlockData(blockData, { common: this.common });
    }
    ...
    if (this.DEBUG) {
        debug(`Running block finished hash=${(0, util_1.bytesToHex)(block.hash())} number=${block.header.number} hardfork=${this.common.hardfork()}`);
    }
    if (enableProfiler) {
        // eslint-disable-next-line no-console
...
async function accumulateParentBeaconBlockRoot(root, timestamp) {
    ...
    await this.stateManager.putContractStorage(parentBeaconBlockRootAddress, (0, util_1.setLengthLeft)((0, util_1.bigIntToBytes)(timestampIndex), 32), (0, util_1.bigIntToBytes)(timestamp));
    await this.stateManager.putContractStorage(parentBeaconBlockRootAddress, (0, util_1.setLengthLeft)((0, util_1.bigIntToBytes)(timestampExtended), 32), root);
}
...
async function assignWithdrawals(block) {
