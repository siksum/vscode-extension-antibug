...
class VerkleTree {
    ...
    constructor(opts) {
        ...
        this.EMPTY_TREE_ROOT = (0, util_1.zeros)(32);
    ...
    database(db) {
        if (db !== undefined) {
            if (db instanceof checkpoint_js_1.CheckpointDB) {
                throw new Error('Cannot pass in an instance of CheckpointDB');
            }
            this._db = new checkpoint_js_1.CheckpointDB({ db, cacheSize: this._opts.cacheSize });
        }
        return this._db;
    }
    /**
    ...
    async put(key, value) {
        ...
        while (currentDepth > 0) {
            ...
            const parentNode = internalNode_js_1.InternalNode.create(currentDepth);
    ...
    async findPath(key, throwIfMissing = false) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const onFound = async (_, node, keyProgress, walkController) => {
                ...
                const keyRemainder = key.slice((0, index_js_1.matchingBytesLength)(keyProgress, key));
                stack.push(node);
                if (node instanceof internalNode_js_1.InternalNode) {
     ...
     */
    async walkTree(root, onFound) {
        await index_js_1.WalkController.newWalk(onFound, this, root);
    }
    /**
     ...
     * @param throwIfMissing - if true, throws if any nodes are missing. Used for verifying proofs. (default: false)
     */
    async findLeafNode(key, throwIfMissing = false) {
        const { node } = await this.findPath(key, throwIfMissing);
        if (!(node instanceof leafNode_js_1.LeafNode)) {
            if (throwIfMissing) {
    ...
    async lookupNode(node) {
        if ((0, util_js_1.isRawNode)(node)) {
            return (0, util_js_1.decodeRawNode)(node);
        }
        const value = await this._db.get(node);
        if (value !== undefined) {
            return (0, util_js_1.decodeNode)(value);
        }
        else {
    ...
    async persistRoot() {
        if (this._opts.useRootPersistence) {
            await this._db.put(types_js_1.ROOT_DB_KEY, this.root());
        }
    }
    /**
