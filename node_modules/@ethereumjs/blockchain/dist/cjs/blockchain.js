...
const index_js_1 = require("./consensus/index.js");
const helpers_js_1 = require("./db/helpers.js");
const manager_js_1 = require("./db/manager.js");
const operation_js_1 = require("./db/operation.js");
/**
...
async function genGenesisStateRoot(genesisState, common) {
    ...
    if (genCommon.isActivatedEIP(6800)) {
        ...
        return (0, trie_1.genesisStateRoot)(genesisState);
    }
}
/**
...
class Blockchain {
    ...
    constructor(opts = {}) {
        ...
        this.db = opts.db !== undefined ? opts.db : new util_1.MapDB();
        this.dbManager = new manager_js_1.DBManager(this.db, this.common);
        ...
        if (opts.consensus) {
            ...
            switch (this.common.consensusAlgorithm()) {
                case common_1.ConsensusAlgorithm.Casper:
                    this.consensus = new index_js_1.CasperConsensus();
                    break;
                case common_1.ConsensusAlgorithm.Clique:
                    this.consensus = new index_js_1.CliqueConsensus();
                    break;
                case common_1.ConsensusAlgorithm.Ethash:
                    this.consensus = new index_js_1.EthashConsensus();
                    break;
                default:
    ...
    static async create(opts = {}) {
        ...
        if (!dbGenesisBlock) {
            ...
            const dbOps = [];
            dbOps.push((0, helpers_js_1.DBSetTD)(genesisBlock.header.difficulty, util_1.BIGINT_0, genesisHash));
            (0, helpers_js_1.DBSetBlockOrHeader)(genesisBlock).map((op) => dbOps.push(op));
            (0, helpers_js_1.DBSaveLookups)(genesisHash, util_1.BIGINT_0).map((op) => dbOps.push(op));
        ...
        await this.runWithLock(async () => {
            ...
            try {
                ...
                if (isGenesis) {
                    ...
                    throw new Error('Cannot put a different genesis block than current blockchain genesis: create a new Blockchain');
                }
                const { header } = block;
                ...
                if (block.common.chainId() !== this.common.chainId()) {
                    throw new Error(`Chain mismatch while trying to put block or header. Chain ID of block: ${block.common.chainId}, chain ID of blockchain : ${this.common.chainId}`);
                }
                ...
                dbOps = dbOps.concat((0, helpers_js_1.DBSetTD)(td, blockNumber, blockHash));
                // save header/block to the database
                dbOps = dbOps.concat((0, helpers_js_1.DBSetBlockOrHeader)(block));
                let commonAncestor;
                let ancestorHeaders;
                ...
                if (block.isGenesis() ||
                    ...
                    dbOps.push((0, helpers_js_1.DBSetHashToNumber)(blockHash, blockNumber));
                }
                const ops = dbOps.concat(this._saveHeadOps());
                await this.dbManager.batch(ops);
    ...
    async getBlock(blockId) {
        ...
        if (block === undefined) {
            if (typeof blockId === 'object') {
                throw new Error(`Block with hash ${(0, util_1.bytesToHex)(blockId)} not found in DB`);
            }
            else {
    ...
    async getTotalDifficulty(hash, number) {
        if (number === undefined) {
            ...
            if (number === undefined) {
                throw new Error(`Block with hash ${(0, util_1.bytesToHex)(hash)} not found in DB`);
            }
        }
    ...
    async _delChild(blockHash, blockNumber, headHash, ops) {
        ...
        ops.push(helpers_js_1.DBOp.del(operation_js_1.DBTarget.Header, { blockHash, blockNumber }));
        ops.push(helpers_js_1.DBOp.del(operation_js_1.DBTarget.Body, { blockHash, blockNumber }));
        ops.push(helpers_js_1.DBOp.del(operation_js_1.DBTarget.HashToNumber, { blockHash }));
        ops.push(helpers_js_1.DBOp.del(operation_js_1.DBTarget.TotalDifficulty, { blockHash, blockNumber }));
        if (!headHash) {
            return;
        }
    ...
    async _deleteCanonicalChainReferences(blockNumber, headHash, ops) {
        try {
            ...
            while (hash !== false) {
                ops.push(helpers_js_1.DBOp.del(operation_js_1.DBTarget.NumberToHash, { blockNumber }));
    ...
    async _rebuildCanonical(header, ops) {
        ...
        while (await loopCondition()) {
            ...
            (0, helpers_js_1.DBSaveLookups)(blockHash, blockNumber).map((op) => {
                ops.push(op);
            });
    ...
    _saveHeadOps() {
        ...
        return [
            helpers_js_1.DBOp.set(operation_js_1.DBTarget.Heads, hexHeads),
            helpers_js_1.DBOp.set(operation_js_1.DBTarget.HeadHeader, this._headHeaderHash),
            helpers_js_1.DBOp.set(operation_js_1.DBTarget.HeadBlock, this._headBlockHash),
        ];
    }
    /**
    ...
    async _getHeader(hash, number) {
        if (number === undefined) {
            number = await this.dbManager.hashToNumber(hash);
            if (number === undefined)
                throw new Error(`no header for ${(0, util_1.bytesToHex)(hash)} found in DB`);
        }
        return this.dbManager.getHeader(hash, number);
    }
    async checkAndTransitionHardForkByNumber(number, td, timestamp) {
        ...
        if (!Object.values(common_1.ConsensusAlgorithm).includes(this.consensus.algorithm))
            return;
        switch (this.common.consensusAlgorithm()) {
            case common_1.ConsensusAlgorithm.Casper:
                if (!(this.consensus instanceof index_js_1.CasperConsensus)) {
                    this.consensus = new index_js_1.CasperConsensus();
                }
                break;
            case common_1.ConsensusAlgorithm.Clique:
                if (!(this.consensus instanceof index_js_1.CliqueConsensus)) {
                    this.consensus = new index_js_1.CliqueConsensus();
                }
                break;
            case common_1.ConsensusAlgorithm.Ethash:
                if (!(this.consensus instanceof index_js_1.EthashConsensus)) {
                    this.consensus = new index_js_1.EthashConsensus();
                }
                break;
            default:
    ...
    createGenesisBlock(stateRoot) {
        ...
        return block_1.Block.fromBlockData({ header, withdrawals: common.isActivatedEIP(4895) ? [] : undefined }, { common });
    }
}
