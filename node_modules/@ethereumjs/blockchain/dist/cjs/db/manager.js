...
class DBManager {
    ...
    async getBlock(blockId) {
        ...
        if (number === util_1.BIGINT_0) {
            opts.setHardfork = await this.getTotalDifficulty(hash, util_1.BIGINT_0);
        }
        else {
            opts.setHardfork = await this.getTotalDifficulty(header.parentHash, number - util_1.BIGINT_1);
        }
    ...
    async getHeader(blockHash, blockNumber) {
        ...
        if (blockNumber === util_1.BIGINT_0) {
            opts.setHardfork = await this.getTotalDifficulty(blockHash, util_1.BIGINT_0);
        }
        else {
            // Lets fetch the parent hash but not by number since this block might not
            // be in canonical chain
            const headerData = (0, block_1.valuesArrayToHeaderData)(headerValues);
            const parentHash = headerData.parentHash;
            opts.setHardfork = await this.getTotalDifficulty(parentHash, blockNumber - util_1.BIGINT_1);
        }
        return block_1.BlockHeader.fromValuesArray(headerValues, opts);
    }
    /**
    ...
    async hashToNumber(blockHash) {
        ...
        if (value === undefined) {
            throw new Error(`value for ${(0, util_1.bytesToHex)(blockHash)} not found in DB`);
        }
