...
class EVM {
    constructor(opts = {}) {
        ...
        if (opts.common) {
            ...
            this.common = new common_1.Common({ chain: DEFAULT_CHAIN });
        }
        let blockchain;
        if (opts.blockchain === undefined) {
            blockchain = new types_js_1.DefaultBlockchain();
        }
        else {
            blockchain = opts.blockchain;
        }
        this.blockchain = blockchain;
        this.stateManager = opts.stateManager ?? new statemanager_1.DefaultStateManager();
        // Supported EIPs
        const supportedEIPs = [
     ...
     */
    getActiveOpcodes() {
        const data = (0, index_js_1.getOpcodesForHF)(this.common, this._customOpcodes);
    ...
    async _executeCall(message) {
        ...
        if (!toAccount) {
            if (this.common.isActivatedEIP(6800)) {
                ...
                if (gasLimit < util_1.BIGINT_0) {
                    if (this.DEBUG) {
                        debugGas(`Proof of absense access charged(${absenceProofAccessGas}) caused OOG (-> ${gasLimit})`);
                    }
                    return { execResult: OOGResult(message.gasLimit) };
                }
                else {
        ...
        if (message.isCompiled) {
            ...
            if (this._optsCached.profiler?.enabled === true) {
                ...
                target = (0, index_js_2.getPrecompileName)(target) ?? target.slice(20);
    ...
    async _executeCreate(message) {
        ...
        let result = await this.runInterpreter({ ...message, gasLimit });
        result.executionGasUsed += message.gasLimit - gasLimit;
        ...
        if (totalGas <= message.gasLimit && (this.allowUnlimitedContractSize || allowedCodeSize)) {
            if (this.common.isActivatedEIP(3541) && result.returnValue[0] === eof_js_1.EOF.FORMAT) {
                if (!this.common.isActivatedEIP(3540)) {
                    result = { ...result, ...INVALID_BYTECODE_RESULT(message.gasLimit) };
                }
                // Begin EOF1 contract code checks
                // EIP-3540 EOF1 header check
                const eof1CodeAnalysisResults = eof_js_1.EOF.codeAnalysis(result.returnValue);
                if (typeof eof1CodeAnalysisResults?.code === 'undefined') {
                    ...
                    if (!eof_js_1.EOF.validOpcodes(result.returnValue.subarray(codeStart, codeStart + eof1CodeAnalysisResults.code))) {
                        result = {
                            ...result,
                            ...INVALID_EOF_RESULT(message.gasLimit),
                        };
    ...
    async runInterpreter(message, opts = {}) {
        let contract = await this.stateManager.getAccount(message.to ?? util_1.Address.zero());
        if (!contract) {
        ...
        const interpreter = new interpreter_js_1.Interpreter(this, this.stateManager, this.blockchain, env, message.gasLimit, this.journal, this.performanceLogger, this._optsCached.profiler);
    ...
    async runCall(opts) {
        ...
        if (!message) {
            ...
            this._tx = {
                ...
                origin: opts.origin ?? opts.caller ?? util_1.Address.zero(),
            };
        ...
        if (err &&
            !(this.common.hardfork() === common_1.Hardfork.Chainstart && err.error === exceptions_js_1.ERROR.CODESTORE_OUT_OF_GAS)) {
    ...
    async runCode(opts) {
        ...
        const message = new message_js_1.Message({
            ...
            data: opts.data,
            gasLimit: opts.gasLimit ?? BigInt(0xffffff),
            to: opts.to ?? util_1.Address.zero(),
            caller: opts.caller,
            value: opts.value,
            depth: opts.depth,
            selfdestruct: opts.selfdestruct ?? new Set(),
    ...
    async _loadCode(message) {
        if (!message.code) {
            ...
            if (precompile) {
                ...
                if (this.common.isActivatedEIP(3540)) {
                    message.code = (0, eof_js_1.getEOFCode)(message.containerCode);
                }
                else {
