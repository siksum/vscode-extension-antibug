...
class Interpreter {
    ...
    async run(code, opts = {}) {
        if (!this.common.isActivatedEIP(3540) || code[0] !== eof_js_1.EOF.FORMAT) {
            ...
            if (code[2] !== eof_js_1.EOF.VERSION) {
            ...
            // Code is EOF1 format
            const codeSections = eof_js_1.EOF.codeAnalysis(code);
            if (!codeSections) {
        ...
        while (this._runState.programCounter < this._runState.code.length) {
            ...
            try {
                ...
                if (e.error !== exceptions_js_1.ERROR.STOP) {
                    err = e;
                }
                break;
            }
    ...
    async runStep(opcodeObj) {
        ...
        try {
            ...
            if (opInfo.isInvalid) {
                throw new exceptions_js_1.EvmError(exceptions_js_1.ERROR.INVALID_OPCODE);
            }
        ...
        for (let i = 0; i < code.length; i++) {
            ...
            if (opcode <= 0x7f) {
                if (opcode >= 0x60) {
                    ...
                    const push = (0, util_1.bytesToBigInt)(code.slice(i + 1, i + opcode - 0x5e));
                    pushes[i + 1] = push;
                    i += extraSteps;
                }
                else if (opcode === 0x5b) {
    ...
    useGas(amount, context) {
        ...
        if (this._runState.gasLeft < util_1.BIGINT_0) {
            ...
            (0, index_js_1.trap)(exceptions_js_1.ERROR.OUT_OF_GAS);
        }
    }
    /**
    ...
    subRefund(amount, context) {
        ...
        if (this._runState.gasRefund < util_1.BIGINT_0) {
            ...
            (0, index_js_1.trap)(exceptions_js_1.ERROR.REFUND_EXHAUSTED);
        }
    }
    /**
    ...
    revert(returnData) {
        this._result.returnValue = returnData;
        (0, index_js_1.trap)(exceptions_js_1.ERROR.REVERT);
    }
    /**
    ...
    async _baseCall(msg) {
        ...
        if (results.execResult.returnValue !== undefined &&
            (!results.execResult.exceptionError ||
                results.execResult.exceptionError.error === exceptions_js_1.ERROR.REVERT)) {
    ...
    async create(gasLimit, value, data, salt) {
        ...
        if (!results.execResult.exceptionError ||
            results.execResult.exceptionError.error === exceptions_js_1.ERROR.CODESTORE_OUT_OF_GAS) {
            for (const addressToSelfdestructHex of selfdestruct) {
                this._result.selfdestruct.add(addressToSelfdestructHex);
            }
            if (this.common.isActivatedEIP(6780)) {
    ...
    log(data, numberOfTopics, topics) {
        if (numberOfTopics < 0 || numberOfTopics > 4) {
            (0, index_js_1.trap)(exceptions_js_1.ERROR.OUT_OF_RANGE);
        }
        if (topics.length !== numberOfTopics) {
            (0, index_js_1.trap)(exceptions_js_1.ERROR.INTERNAL_ERROR);
        }
